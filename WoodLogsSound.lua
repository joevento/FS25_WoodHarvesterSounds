...
--
WoodLogsSound = {};
WoodLogsSound.Dir = g_currentModDirectory;
		
function WoodLogsSound.prerequisitesPresent(specializations)
    return SpecializationUtil.hasSpecialization(Motorized, specializations)
end;

function WoodLogsSound:preLoad(savegame)
end;

function WoodLogsSound:load(savegame)
	self.wls = {}
	...
	self.triggerCallback = SpecializationUtil.callSpecializationsFunction("triggerCallback"); 
	self.playSound = SpecializationUtil.callSpecializationsFunction("playSound"); 
	self.playSoundGround = SpecializationUtil.callSpecializationsFunction("playSoundGround"); 
	self.playSoundFall = SpecializationUtil.callSpecializationsFunction("playSoundFall"); 
	self.checkPlayingSamples = SpecializationUtil.callSpecializationsFunction("checkPlayingSamples"); 
	self.checkIsInRange = SpecializationUtil.callSpecializationsFunction("checkIsInRange"); 
	
	self.wls.trigger = loadI3DFile(Utils.getFilename("trigger.i3d", WoodLogsSound.Dir));
	local triggerId = getChildAt(self.wls.trigger,0)
	self.wls.trigger = triggerId
	addTrigger(triggerId, "triggerCallback", self);
	link(self.wls.root, triggerId)
...
function WoodLogsSound:delete()
	if self.wls.trigger then
		removeTrigger(self.wls.trigger)
	end
end;

function WoodLogsSound:mouseEvent(posX, posY, isDown, isUp, button)
end;

function WoodLogsSound:keyEvent(unicode, sym, modifier, isDown)
end;

function WoodLogsSound:update(dt)
...
function WoodLogsSound:updateTick(dt)
end

function WoodLogsSound:triggerCallback(triggerId,otherId,onEnter,onLeave,onStay)
	if onEnter then
...
function WoodLogsSound:playSound(isPlaying)
	if g_currentMission.wls.samplesLogs ~= nil then
		if isPlaying then
...
function WoodLogsSound:playSoundGround(isPlaying)
...
function WoodLogsSound:playSoundFall(isPlaying)
...
function WoodLogsSound:checkPlayingSamples(samples)
...
function WoodLogsSound:checkIsInRange(shape,shape2)
	local inRange = false
	local target
...
function WoodLogsSound:readStream(streamId, connection)
end;

function WoodLogsSound:writeStream(streamId, connection)
end;

function WoodLogsSound:readUpdateStream(streamId, timestamp, connection)
end;

function WoodLogsSound:writeUpdateStream(streamId, connection, dirtyMask)
end;

function WoodLogsSound:draw()
	if self.isClient then
		if self:getIsActive() then
			if self.wls.debug == true then
				...
				if g_currentMission.controlledVehicle ~= nil then
					target = g_currentMission.controlledVehicle.rootNode
					local x,y,z = getWorldTranslation(target)
					local nx, ny, nz = localDirectionToWorld(target, 8, 0, 0)
					local yx, yy, yz = localDirectionToWorld(target, 0, 8, 0)
					local zx, zy, zz = localDirectionToWorld(target, 0, 0, 8)
					drawDebugLine(x,y,z, 1, 0, 0, x + nx, y + ny, z + nz, 1, 0, 0);
					drawDebugLine(x,y,z, 0, 1, 0, x + yx, y + yy, z + yz, 0, 1, 0);
					drawDebugLine(x,y,z, 0, 0, 1, x + zx, y + zy, z + zz, 0, 0, 1);
				end
...
function WoodLogsSound:onAttach()
end; 

function WoodLogsSound:onDetach()
end;

function WoodLogsSound:onLeave()
end

function WoodLogsSound:getSaveAttributesAndNodes(nodeIdent)
	local attributes = "";
    local nodes = "";
	return attributes,nodes;
end

function WoodLogsSound:loadFromAttributesAndNodes(xmlFile, key, resetVehicles)
